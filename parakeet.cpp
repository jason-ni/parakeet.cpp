...
{
    return norm_feed_forward1->tensor_count() + feed_forward1->tensor_count();
}
...
ggml_runtime::TensorBag ConFormerLayer::build_graph(ggml_runtime::Session* session, ggml_runtime::TensorBag input_tensors, ggml_runtime::TensorContainer* session_tensor_container)
{
    ...
    auto norm_feed_forward1_output = norm_feed_forward1->build_graph(session, input_tensors, session_tensor_container);
    auto feed_forward1_output = feed_forward1->build_graph(session, norm_feed_forward1_output, session_tensor_container);
    return feed_forward1_output;
}
